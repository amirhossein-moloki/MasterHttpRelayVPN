...
### فایل اجرایی پورتیبل (ساخت نسخه برای انتشار)

اگر می‌خواهید یک فایل اجرایی تک‌فایلی (portable) برای خود یا دیگران بسازید:

```bash
python3 scripts/build_exe.py
```
این دستور از PyInstaller استفاده می‌کند تا برنامه و تمام متعلقات آن (از جمله آیکون) را در یک فایل واحد در پوشه `dist/` بسته‌بندی کند.
...
### بهینه‌سازی اختصاصی برای اینترنت ایران (DPI Bypass) 🇮🇷

این پروژه به صورت پیش‌فرض دارای لایه‌ی بهینه‌سازی سوکت برای عبور از محدودیت‌های شدید (DPI) ایران است:

۱. **کاهش Fragmentation (تنظیم MSS):** برای جلوگیری از قطعی پکت‌ها در شبکه‌هایی که PMTU discovery آن‌ها مختل شده، سایز قطعات ارسالی (MSS) به صورت خودکار روی ۱۳۴۰ تنظیم می‌شود. این کار باعث افزایش پایداری در بارگذاری صفحات سنگین می‌شود.
۲. **استفاده از TCP Fast Open (TFO):** برای کاهش Latency و حذف یک مرحله از Handshake در اتصالات (در صورت پشتیبانی سیستم‌عامل).
۳. **رفع Buffer Bloat:** با استفاده از تنظیم `TCP_NOTSENT_LOWAT` میزان داده‌های انباشته شده در بافر سوکت کاهش یافته و پینگ ارتباط پایدارتر می‌ماند.
۴. **چرخش SNI (SNI Rotation):** برای جلوگیری از شناسایی الگو توسط DPI، در هر اتصال از یکی از دامنه‌های معتبر گوگل به عنوان ماسک استفاده می‌شود.
...
## راهنمای کامل Routing Rules (قوانین مسیریابی) 🛣️

سیستم **Routing Rules** به شما اجازه می‌دهد تعیین کنید که ترافیک هر سایت یا آی‌پی چگونه مدیریت شود. این سیستم جایگزین هوشمندتری برای Bypass Hosts قدیمی است.

### حالت‌های اتصال (Connection Modes)
هر قانون یا گروهی از قوانین می‌تواند یکی از سه حالت زیر را داشته باشد:
1.  **Direct (مستقیم):** ترافیک بدون عبور از رله و بدون تغییر (بدون MITM) مستقیماً ارسال می‌شود. مناسب برای سایت‌های ایرانی یا سایت‌هایی که با پروکسی مشکل دارند.
2.  **Relay (رله):** ترافیک از طریق Google Apps Script و رله شما عبور می‌کند. برای دور زدن فیلترینگ استفاده می‌شود.
3.  **Block (مسدود):** ترافیک به طور کامل مسدود می‌شود. مناسب برای جلوگیری از تبلیغات یا ردیاب‌ها.

### نحوه اضافه کردن دامنه و آی‌پی
در محیط گرافیکی (GUI)، به تب **Routing Rules** بروید:

#### استفاده از Quick Add (افزودن سریع):
این دکمه ساده‌ترین راه برای اضافه کردن یک مورد جدید است:
- **Domain/IP:** نام دامنه (مثل `apple.com`) یا آی‌پی (مثل `1.1.1.1`) را وارد کنید.
- **Mode:** حالت مورد نظر (Direct, Relay, Block) را انتخاب کنید.
- دامنه‌ها به صورت خودکار شامل تمام زیردامنه‌ها نیز می‌شوند. مثلاً با اضافه کردن `google.com` در حالت Direct، سایت `mail.google.com` هم مستقیم باز می‌شود.

#### مدیریت گروه‌ها (Add Group):
می‌توانید مجموعه‌ای از قوانین را در یک گروه دسته‌بندی کنید:
- **Group Name:** یک نام برای گروه خود انتخاب کنید (مثلاً "Iranian Sites").
- **Mode:** حالتی که می‌خواهید روی تمام موارد این گروه اعمال شود.
- **Rules:** لیست دامنه‌ها یا آی‌پی‌ها را وارد کنید (هر کدام در یک خط).

### مثال‌های کاربردی
-   **افزودن دامنه:** `example.ir` یا `pishkhan.com`
-   **افزودن کل پسوند دامنه‌ها:** با وارد کردن `.ir` در لیست قوانین، تمام سایت‌هایی که با این پسوند تمام می‌شوند (بدون استثنا) شامل آن قانون می‌شوند.
-   **افزودن آی‌پی تکی:** `185.123.45.6`
-   **افزودن رنج آی‌پی (CIDR):** `192.168.1.0/24` یا `10.0.0.0/8`

### اولویت قوانین
برنامه ابتدا لیست گروه‌ها را از بالا به پایین چک می‌کند. اولین قانونی که با مقصد شما مطابقت داشته باشد، اعمال خواهد شد. اگر هیچ قانونی پیدا نشود، از حالت پیش‌فرض (`default_connection_mode`) که در تنظیمات تعیین کرده‌اید استفاده می‌شود.
