...
- ⚙️ **تنظیمات پیشرفته (Professional Settings):** طراحی ماژولار و دسته‌بندی‌شده (شبکه، بای‌پس، ذخیره تنظیمات) با امکان ذخیره و بازیابی چندین پروفایل تنظیمات (Presets).
- 🌐 **بای‌پس کامل آی‌پی‌های ایران:** سیستم هوشمند شناسایی و بای‌پس خودکار تمامی رنج‌های آی‌پی ایران (GeoIP)، حتی برای وب‌سایت‌هایی با دامنه غیر ایرانی که در داخل کشور میزبانی می‌شوند.
- ✍️ **بای‌پس دستی (Manual Bypass):** قابلیت تعریف لیست شخصی از آی‌پی‌ها یا رنج‌های CIDR برای عبور مستقیم (بدون پراکسی) با اولویت بالا.
- 📊 **مانیتورینگ مصرف (Usage Monitoring):** داشبورد کامل برای مشاهده مصرف به تفکیک آپلود و دانلود، نمودارهای گرافیکی، فیلترهای زمانی (روزانه، هفتگی، ماهانه) و جدول ۱۰ مورد پرمصرف.
...
python3 gui.py
```
رابط گرافیکی امکاناتی شامل داشبورد وضعیت، مدیریت تنظیمات، مشاهده لاگ‌ها و اسکنر داخلی IP های گوگل را فراهم می‌کند.
...
python3 main.py --gui                 # اجرا به صورت گرافیکی
```
...
- هر دیپلویمنت روی گوگل اسکریپت دارای محدودیت 20,000 درخواست در هر 24 ساعت است (برنامه به صورت خودکار مجموع محدودیت را بر اساس تعداد Script IDهای وارد شده محاسبه می‌کند).
- دامنه های ایرانی (`.ir`) به صورت خودکار شناسایی شده و بدون عبور از پروکسی (مستقیم) متصل می‌شوند تا سرعت افزایش یافته و از سهمیه مصرف نشود.
---
